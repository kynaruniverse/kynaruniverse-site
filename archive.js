/**
 * SOFT ROYAL ARCHIVE LOGIC
 * Role: Render the digital vault grid and handle filtering
 */

const ArchiveSystem = (() => {

    // --- 1. THE ARTIFACT VAULT (Database) ---
    const ARTIFACTS = [
        {
            id: 'art_001',
    ...
    // --- 2. THE RENDERER (Glass Card Engine) ---
    const Renderer = {
    ...
    const Controller = {
        init() {
            if (DOM.grid) {
                ...
                const filterParam = urlParams.get('filter') || urlParams.get('tag'); // Handle both filter and tag params
        ...
        highlightFilter(filterName) {
            DOM.filters.forEach(b => {
                ...
                // Activate target
                if (b.dataset.filter === filterName || (filterName === 'all' && b.dataset.filter === 'all')) {
