/**
 * ══════════════════════════════════════════════════════════════════════════
 * MODULE: SOFT ROYAL SIGNAL LOGIC
 * ══════════════════════════════════════════════════════════════════════════
 * @description Manages the newsletter subscription (The Signal), simulates
 * live social proof activity, and issues Gatekeeper tokens for free downloads.
 * @module SignalLogic
 */

document.addEventListener("DOMContentLoaded", () => {
  // #region [ 1. LIVE SIGNAL STRENGTH (SOCIAL PROOF) ]

  /**
   * Simulates a live counter of architects currently "tuned in".
   * Updates randomly to create a sense of active community.
   */
  ...
  if (strengthEl) {
    ...
    setInterval(() => {
      ...
      if (Math.random() > 0.7) {
        ...
        // Animate the change visually (Fade Out -> Update -> Fade In)
  ...
  // #region [ 2. TRANSMISSION HANDLER (FORM SUBMIT) ]
  ...
  if (form) {
     ...
     * Simulates a network request, saves the auth token, and redirects.
     */
